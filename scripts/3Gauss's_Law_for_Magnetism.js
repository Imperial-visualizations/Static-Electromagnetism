...
const Nvertices = 1700, max_range = 1500, R = 16, square_size = 100, padding = 50, rect_height = height/8, dipdist=1.5, arrow_size = 5;
...
class dipole_selector{
    ...
    pressed(){
        if (dist(mouseX,mouseY,this.x,this.y)<2*this.r){
            let Q1 = new charge(this.posq,this.x,this.y);
            let Q2 = new charge(this.negq,this.x,this.y);
            activepoints.push(Q1);
            activepoints.push(Q2);
        }
    }
}
...
function draw_fieldlines(initialx, initialy, q){
    ...
    for (let i = 0; i < Nvertices; i++) {
        ...
        line(xfield0-dipdist*R*q, yfield0, xfield1-dipdist*R*q, yfield1);
        if (i == Math.round(Nvertices/12)) {
            line(xfield0 - q*dy*arrow_size-dipdist*R*q, yfield0 + q*dx*arrow_size, xfield0 + arrow_size*q*dx-dipdist*R*q, yfield0 + arrow_size*q*dy);
            line(xfield0 + q*dy*arrow_size-dipdist*R*q, yfield0 - q*dx*arrow_size, xfield0 + arrow_size*q*dx-dipdist*R*q, yfield0 + arrow_size*q*dy);
        }
...
/*
    for (let i = 0; i < activenegpoints.length; i++) {
        ...
        ellipse(activenegpoints[i].x+dipdist*R, activenegpoints[i].y, R*2);
    }

    for (let i = 0; i < activepoints.length; i++) {
        noStroke(1);
        fill(color(activepospoints[i].poscolor));
        ellipse(activepospoints[i].x-dipdist*R, activepospoints[i].y, R*2);
    }
*/
    noStroke();
    fill(247, 252, 251);
    ...
    if (activepoints.length < maxpoints){
        ...
        ellipse(dip.x-dipdist*R, dip.y, R*2);
