 ...
 */
function Circle(radius,op) { //opacity passed as argument
    this.radius = radius;

    this.gObject = function(color=cyan, centre=[0,0]) {
        var phi = numeric.linspace(0, 2*Math.PI, 100);
        var x = [], y = [];
        for (var i=0, n=phi.length; i<100; ++i) {
...
function Line2d(points) {
    ...
    this.gObject = function(color, op=1, width=7, dash="solid", modetype="lines", textInput="") {
        var lineObject = {
            type: "scatter",
            mode: modetype,
            text: ["",textInput],
